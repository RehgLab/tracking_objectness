...
def test_on_fullseq(model, d, sw, iters=8, S_max=8, image_size=(384,512), step = None):
    metrics = {}
    ...
    while not done:
        ...
        preds, preds_anim, feat_init, _, _, _ = model(traj_seq, rgb_seq, iters=iters, feat_init=feat_init)
    ...
    #prep_rgb0 = utils.improc.preprocess_color(rgbs[0:1])
    ...
    #make_video(str(step),giff_pred, pred=True)
...
def main(
    ...
    while global_step < max_iters:
        ...
        #    continue
        with torch.no_grad():
            metrics = test_on_fullseq(model, sample, sw_x, iters=iters, S_max=S, image_size=image_size, step = global_step)
