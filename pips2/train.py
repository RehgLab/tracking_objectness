...
def run_model(model, d, device, iters=8, sw=None, is_train=True, use_augs=True):
    total_loss = torch.tensor([0.0,0.0], requires_grad=True).to(device)
...
def main(
        ...
        dataset_location='/nethome/bboote3/data/bboote3/TrackingTRI/pod_export', # where we exported the data
    ...
    while global_step < max_iters:
        ...
        for internal_step in range(grad_acc):
            ...
            total_loss /= grad_acc
            scaler.scale(total_loss).backward(torch.ones(2).to(device))
        ...
        if val_freq:
            print('%s; step %06d/%d; rtime %.2f; itime %.2f; loss %.3f; loss_t %.2f; d_t %.1f; d_v %.1f' % (
                ...
                total_loss.mean().item(), pools_t['total_loss'].mean(), pools_t['d_avg'].mean(), pools_v['d_avg'].mean()))
        else:
