...
class PointOdysseyDataset_masks(torch.utils.data.Dataset):
    def __init__(self,
    ...
    def getitem_helper(self, index):
        ...
        if self.use_augs and (np.random.rand() < self.spatial_aug_prob):
            rgbs, trajs, masks = self.add_spatial_augs(rgbs, trajs, visibs, masks)
        else:
            # either crop or resize
            if np.random.rand() < 0.5:
                rgbs, trajs, masks = self.just_crop(rgbs, trajs, masks)
            else:
                rgbs, trajs, masks = self.just_resize(rgbs, trajs, masks)
        ...
        rgbs = torch.from_numpy(np.stack(rgbs, 0)).permute(0,3,1,2).byte()
        masks = torch.from_numpy(np.stack(masks, 0)).byte() # S,C,H,W
    ...
    def add_spatial_augs(self, rgbs, trajs, visibs, masks):
        T, N, _ = trajs.shape
        ...
        return rgbs, trajs, masks

    def just_crop(self, rgbs, trajs, masks):
        T, N, _ = trajs.shape
    ...
    def just_resize(self, rgbs, trajs, masks):
        T, N, _ = trajs.shape
